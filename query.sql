...
-- Vārds Uzvārds - ERNESTS RALFS KALNIŅŠ
...
INSERT INTO Pilsetas(title, year, population, region)
VALUES('Ādaži', 2022, 7482, 'Ādažu novads')
...
SELECT * FROM Pilsetas WHERE year < 1924;

SELECT * FROM Pilsetas WHERE population <= 3501;

SELECT * From Pilsetas ORDER BY population DESC LIMIT 10;

SELECT COUNT(title) FROM Pilsetas;

SELECT COUNT(DISTINCT(region)) FROM Pilsetas;

DELETE FROM Pilsetas WHERE title = 'Durbe';
SELECT * FROM Pilsetas;

SELECT region,SUM(population) FROM Pilsetas GROUP BY region;
